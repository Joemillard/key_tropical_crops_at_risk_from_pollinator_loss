...
sampling_methods <- PREDICTS_pollinators_orig %>% 
  group_by(Sampling_method) %>%
  tally() %>% arrange(desc(n)) %>% slice(0:10) %>% pull(Sampling_method)
...
# list object for each of the plots
abundance_model <- list()

# iterate through each of the sampling method types
for(j in 1:length(sampling_methods)){
  
  # filter for each of top 4 sampling methods in turn
  PREDICTS_pollinators_filt <- PREDICTS_pollinators %>%
    filter(!Sampling_method %in% sampling_methods[j]) %>% droplevels()
  
  # calculate site metrics including all species (confirmed and not confirmed pollinator)
  order.sites.div <- SiteMetrics(diversity = PREDICTS_pollinators_filt,
  ...
  abundance_model[[j]] <- predict_continuous(model = model_abundance_anom,
    ...
    mutate(Sampling_method_dropped = sampling_methods[j])
  
}

all_abundance_data <- rbindlist(abundance_model)

# plot for standardised anomaly and land-use for abundance
abundance_plot_list <- ggplot(all_abundance_data) +
  geom_line(aes(x = standard_anom, y = y_value, colour = Predominant_land_use, group = interaction(Sampling_method_dropped, Predominant_land_use)), alpha = 0.5, size = 1.5) +
