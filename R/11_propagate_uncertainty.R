# code for propagating uncertainty out from all the parameter space
...
# set up vector of abundance_production
...
RCP_plot <- list()
abundance_production_plot <- list()

for(m in 1:length(abundance_prod)){

  print(abundance_prod[m])
  ...
  abundance_production_plot[[m]] <- rbindlist(RCP_plot) %>%
    mutate(relationship = abundance_prod[m])
  
}

# bind together all production risk values
bound_production_risk <- rbindlist(abundance_production_plot)

# split production risk by year
bound_production_risk_year <- split(bound_production_risk, f = bound_production_risk$year)
