...
for(i in 1:length(cropdirs)){
  crop_files_quality[[i]] <- list.files(cropdirs[i], pattern = "DataQuality_Yield.tif$")
  crop_files_quality[[i]] <- paste0(cropdirs[i], "/", crop_files_quality[[i]])
}

# file paths for each of per hectare application, and data quality
unlisted_crops <- unlist(crop.files)
unlisted_quality <- unlist(crop_files_quality)

# set up empty lists for production rasters and data quality
rate_rasters <- list()
quality_rasters <- list()

# subset the file paths for just those that are pollination dependent to some extent, and for data quality
...
pollinated_crops_quality <- grep(paste(unique(paste("/", klein_cleaned$MonfredaCrop, "_", sep = "")), collapse = "|"), unlisted_quality, value = TRUE)
...
RCP_plot %>%
  ...
  mutate(scenario = factor(scenario, levels = c("rcp85"), labels = c("RCP 8.5"))) %>%
  group_by(quality) %>%
  arrange(year) %>%
