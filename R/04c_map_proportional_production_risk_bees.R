...
PREDICTS_pollinators_bees <- PREDICTS_pollinators_orig %>%
  dplyr::filter(Predominant_land_use %in% c("Cropland", "Primary vegetation")) %>%
  dplyr::filter(Phylum %in% "Arthropoda") %>%
  dplyr::filter(Family %in% c("Apidae", "Megachilidae", "Halictidae", "Andrenidae", "Colletidae", "Melittidae", "Stenotritidae")) %>%
  droplevels()
...
PREDICTS_pollinators_bees <- CorrectSamplingEffort(PREDICTS_pollinators_bees)

# calculate site metrics including all species (confirmed and not confirmed pollinator)
order.sites.div <- SiteMetrics(diversity = PREDICTS_pollinators_bees,
...
ggsave("vulnerability_weighted_production_map_bees.png", scale = 1.1, dpi = 350)
